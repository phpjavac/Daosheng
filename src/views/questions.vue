    ...
    <div class="main">
      ...
      <div v-if="mainPageData.length">
        <div v-for="(item, index) in mainPageData" :key="index">
          <div class="title">
            {{
              `子卷体系${Number(index) + 1}（共${item.question.length}道题）`
            }}
          </div>
          <div
            class="content"
          ...
          >
            <div v-if="items.type === '单选题'">
              <SingleChoice
                :singlePropData="items"
                v-model:answer="items.answer"
              />
            </div>
            ...
            <div v-if="items.type === '填空题'">
              <InsertQuestion
                :insertPropData="items"
                v-model:answer="items.answer"
              />
            </div>
            <div v-if="items.type === '判断题'">
              <JustifyChoice
                :justifyPropData="items"
                v-model:answer="items.answer"
              />
            </div>
          </div>
...
import { useRequest } from "../../packages/composition/src/useRequest/index";
